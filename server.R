...
gaussfunc <- Vectorize(function(x, peak, centre, width) {
  y = peak*exp(-(x-centre)^2/(2*width^2))
})
...
output$distPlot  <- renderPlot({
    ...
    theme(axis.text = element_text(size = 12)) + ylab("Workforce") + xlab("Time/Months")
    })
...
output$factoryPlot  <- renderPlot({
  ggplot(data = plot_df_factory(), aes(x=time, y=value, color = discipline), group = discipline) + geom_line(size = 1.5) + 
    theme(axis.text = element_text(size = 12)) + ylab("Workforce") + xlab("Time/Months") + ylim(0, 1000)
    })
