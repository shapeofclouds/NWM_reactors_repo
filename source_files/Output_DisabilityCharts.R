...
output$disabilityPlot <- renderPlot({validate(
  need(nrow(disability_plot())>0, "No data to show for these selections")
)
  ...
  plotMainBar(df_abs = disability_plot(), 
              df_perc = disability_plot(), 
              x="Characteristic", 
              xLables = disabilityOffset,
              percentFlag = percentFlag,
              fillCol = blue, 
              abLabel = "Headcount", 
              percLabel = "Percentage of total", 
              plotTitle = "Disability") +
    bar_theme
})
