...
output$ethnicityPlot <- renderPlot({validate(
  need(nrow(ethnicity_plot())>0, "No data to show for these selections")
)
  ...
  plotMainBar(df_abs = ethnicity_plot(), 
              df_perc = ethnicity_plot(), 
              x="Characteristic", 
              xLables = ethnicityOffset,
...
output$EDI_Data <- renderTable({
  

  
  if (input$EDI_table == "Ethnicity") {
    
      ethnicity_plot() %>%
      select(Characteristic, value_perc) %>%
      mutate(value_perc = value_perc *100) %>%
      dplyr::rename(Percentage = value_perc) %>%
      arrange(Characteristic, desc(Percentage))
    
  } else if (input$EDI_table == "Disability") {
