...
output$agePlot <- renderPlot({validate(
  need(nrow(age_plot())>0, "No data to show for these selections")
)
  if(input$percent_age == "Percentage") {
    percentFlag = TRUE
  } else {
    percentFlag = FALSE
  }
  
  plotMainBar(df_abs = age_plot(), 
              df_perc = age_plot(), 
              x="Age", 
              xLables = ageLabelsOffset,
              percentFlag = percentFlag,
              fillCol = input$ageColour, 
...
output$agePlot_RCT <- renderPlot({
  ...
  plotFacetBar(df = age_plot_RCT(), 
               ...
               yLabel="Workforce/FTEs",
               facetVar = "Function") + #can be NULL for single plot
    bar_theme_facet1
