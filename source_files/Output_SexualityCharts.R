...
output$sexualityPlot <- renderPlot({validate(
  need(nrow(sexuality_plot())>0, "No data to show for these selections")
)
  ...
  plotMainBar(df_abs = sexuality_plot(), 
              df_perc = sexuality_plot(), 
              x="Characteristic", 
              xLables = sexualityOffset,
